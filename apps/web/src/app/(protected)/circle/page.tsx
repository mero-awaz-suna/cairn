...
// Simulated chat messages that tell a realistic story
...
const SESSION_INSIGHTS = [
  { text: "Knowing someone who was exactly where you are made it through", emoji: "🌿", tag: "Hope transfer" },
  { text: "The 'performing okayness' pattern was named and recognized", emoji: "🎭", tag: "Pattern identified" },
  { text: "Carrying weight alone vs. sharing it changes the weight itself", emoji: "🤝", tag: "Core insight" },
];
...
export default function CirclePage() {
  ...
  // Session phase — drip messages
  useEffect(() => {
    if (phase !== "session") return;
    const timer = setInterval(() => setSessionTime((p) => p + 1), 1000);
    const timeouts = SCRIPTED_CHAT.map((msg, i) =>
      setTimeout(() => {
        setVisibleMsgs((prev) => [...prev, msg]);
      }, msg.delay)
    );
    // Wind down after last message
            ...
            <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
              <div>
                ...
                <p className="text-[11px]" style={{ color: "#6B8F71" }}>
                  ...
                  {sessionMins}:{String(sessionSecs).padStart(2, "0")} · 4 members · AI facilitator
                </p>
              </div>
                  ...
                  <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    {msg.type === "facilitator" ? (
                      <div className="p-3.5 rounded-2xl rounded-tl-sm max-w-[85%]" style={{ backgroundColor: "rgba(232,223,211,0.08)", border: "1px solid rgba(232,223,211,0.1)" }}>
                        <p className="text-[10px] font-bold uppercase tracking-[0.1em] mb-1.5" style={{ color: "#D4845A" }}>AI Facilitator</p>
                      ...
                      <div className="flex gap-2.5 items-start">
                        <div className="w-7 h-7 rounded-full flex items-center justify-center text-[11px] flex-shrink-0 mt-0.5"
                          ...
                          {CIRCLE_MEMBERS.find((m) => m.alias === msg.sender)?.emoji || "🌱"}
                        </div>
        ...
        {phase === "summary" && (
          <motion.div key="summary" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <motion.div className="px-6 mb-6" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <p className="text-[13px] font-light" style={{ color: "#8B7E74" }}>4 people · {Math.floor(sessionTime / 60)} minutes · AI facilitated</p>
            </motion.div>
            ...
            <motion.div className="mx-6 mb-5 p-5 rounded-2xl border-l-[3px]"
              ...
              <p className="text-[15px] leading-[1.7]" style={{ color: "#F2EAD8", fontFamily: "var(--font-hand, serif)", fontStyle: "italic" }}>
                &ldquo;Knowing that someone who was exactly where I am made it through — that&apos;s not motivation. That&apos;s evidence.&rdquo;
              </p>
              <p className="text-[11px] mt-2" style={{ color: "#8B7E74" }}>— Steady Peak, Finding Ground</p>
            </motion.div>
            ...
            <motion.div className="mx-6 mb-6 p-4 rounded-xl flex items-center gap-3"
              ...
              <div>
                <p className="text-[12px] font-semibold" style={{ color: "#8FB996" }}>Persona updated → Finding Ground</p>
